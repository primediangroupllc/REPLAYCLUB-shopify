...
const MixExpiringEmail = ({
  ...
  <Html lang="en" dir="ltr">
    <Head />
    <Preview>Your mix "{mixTitle}" expires tomorrow — download it now!</Preview>
    <Body style={main}>
      <Container style={container}>
        <Img src={LOGO_URL} alt={SITE_NAME} width="120" height="40" style={logo} />
        <Heading style={h1}>Your Mix Expires Tomorrow ⏰</Heading>
        <Text style={text}>
          {displayName ? `Hey ${displayName},` : 'Hey,'} just a heads up — your recorded mix will no longer be available for download after tomorrow.
        </Text>
        <Hr style={hr} />
        <Text style={detail}><strong>Title:</strong> {mixTitle}</Text>
        {expiresAt && (
          <Text style={detail}><strong>Expires:</strong> {expiresAt}</Text>
        )}
        <Hr style={hr} />
        <Text style={text}>
          Make sure to download it before it's gone! After expiration, you'll need to contact Replay Club to request access again.
        </Text>
        {profileUrl && (
...
export const template = {
  component: MixExpiringEmail,
  subject: (data: Record<string, any>) =>
    `Your mix "${data.mixTitle || 'recording'}" expires tomorrow — download now`,
  displayName: 'Mix expiring reminder',
  previewData: {
    displayName: 'Jane',
