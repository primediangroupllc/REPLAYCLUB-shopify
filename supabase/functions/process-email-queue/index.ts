// Email is sent via Resend (https://resend.com) — see sendViaResend() below.
...
// Send one queued email via Resend. Throws an Error carrying `.status` so the
// existing 429 (rate-limit retry) and 403/4xx (DLQ) handling keeps working.
async function sendViaResend(payload: Record<string, any>, apiKey: string): Promise<void> {
  ...
  if (payload.unsubscribe_token && siteUrl) {
    const unsubUrl = `${siteUrl.replace(/\/$/, '')}/functions/v1/handle-email-unsubscribe?token=${encodeURIComponent(String(payload.unsubscribe_token))}`
    body.headers = {
  ...
  if (!resp.ok) {
    ...
    const err = new Error(`Resend ${resp.status}: ${detail}`) as Error & { status: number }
